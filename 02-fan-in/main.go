...
func consumer(channels []<-chan payload, done <-chan struct{}, wg *sync.WaitGroup) {
	for i, ch := range channels {
		...
		go func() {
			...
			for {
				select {
				case <-done:
					fmt.Println("consumer", i, "completed")
					return
				case v := <-ch:
					fmt.Println("Consumer", i, "got value", v.value, "from", v.name)
				}
			}
		}()
	}
